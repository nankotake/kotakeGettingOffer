...
- [X] Python 解释器与虚拟环境（venv / conda）
- [X] 基础数据类型：`int`, `float`, `bool`, `str`, `None`
- [X] 变量与动态类型 —— 与 C++ 静态类型的对比
- [X] 输入输出：`input()`, `print()`, f-string 格式化

### 1.2 控制流

- [X] `if / elif / else`（Python 没有 switch，注意缩进）
- [X] `while` 循环
- [X] `for ... in` 迭代（对比 C++ for 循环）
- [X] `range()`, `enumerate()`, `zip()`
- [X] `break`, `continue`, `else` 子句（`for-else` 是 Python 特色）

### 1.3 核心数据结构

- [X] **列表 list**：增删改查、切片（`list[::-1]` 反转）、列表推导式
- [X] **元组 tuple**：不可变序列、拆包
- [X] **字典 dict**：哈希表、`get()`, `setdefault()`, `defaultdict`
- [X] **集合 set**：去重、交并差运算、`frozenset`
- [X] **字符串 str**：常用方法（`split/join/strip/replace/find`）

### 1.4 函数

- [X] 定义与调用 `def`
- [X] 位置参数、默认参数、关键字参数、`*args` / `**kwargs`
- [X] 返回值与多返回值（本质是元组拆包）
- [X] 作用域与 `global` / `nonlocal`

### 1.5 常用内置函数

- [X] `len()`, `range()`, `enumerate()`, `zip()`, `map()`, `filter()`, `sorted()`, `reversed()`
- [X] `any()`, `all()`, `sum()`, `max()`, `min()`
- [X] `isinstance()`, `type()`, `id()`
- [X] `lambda` 匿名函数
...
- [X] `map / filter / reduce`（`reduce` 来自 `functools`）
- [X] 列表推导式 vs `map/filter` 性能对比
- [X] 生成器表达式（惰性求值）：`(x for x in range(10))`
- [X] 闭包与装饰器（**面试必考**）
...
- [X] 可迭代对象 vs 迭代器（`__iter__`, `__next__`）
- [X] `yield` 与生成器函数
- [X] `yield from`
- [X] `itertools` 模块：`chain`, `product`, `permutations`, `combinations`, `groupby`

### 2.3 面向对象编程

- [X] 类定义与 `__init__`
- [X] 实例方法、类方法 `@classmethod`、静态方法 `@staticmethod`
- [X] 属性装饰器 `@property`、`@setter`、`@deleter`
- [X] 魔术方法（`__str__`, `__repr__`, `__len__`, `__getitem__`, `__call__`, `__enter__/__exit__`）
- [X] 继承与 `super()`
- [X] 抽象基类 `ABC` 与 `@abstractmethod`
- [X] 鸭子类型与协议（`__len__` 使对象支持 `len()`）

### 2.4 上下文管理器

- [X] `with` 语句
- [X] 实现上下文管理器：`__enter__` / `__exit__`
- [X] `contextlib.contextmanager` 装饰器方式
...
- [X] 列表作为栈、队列（`collections.deque`）
- [X] `collections.Counter` — 频率统计
- [X] `collections.defaultdict` — 默认值字典
- [X] `collections.OrderedDict` — 有序字典
- [X] `heapq` — 堆（优先队列）
- [X] `bisect` — 二分查找
- [X] 排序：`list.sort()` vs `sorted()`、自定义 key

### 3.2 Python 实现算法模板（对比 C++）

- [X] 快速排序、归并排序（Python 实现）
- [X] 二叉树遍历（递归 vs 迭代）
- [X] 图的 DFS / BFS
- [X] 动态规划（备忘录 vs 自底向上）
- [X] 并查集 Union-Find
...
- [X] 正则表达式 `re` 模块（`search`, `match`, `findall`, `sub`, `split`）
- [X] 字符串比大小的底层原理
